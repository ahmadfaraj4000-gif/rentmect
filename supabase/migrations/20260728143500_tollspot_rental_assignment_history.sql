-- Preserve TollSpot attribution when an administrator swaps a rental vehicle.
...
begin
  ...
  -- Assignment history is authoritative for rentals that have it. The fallback
  -- keeps legacy rentals safe until their initial assignment has been seeded.
  select count(*), min(candidates.rental_id::text)::uuid
  into v_candidate_count, v_rental_id
  from (
    select distinct assignments.rental_id
    from public.rental_vehicle_assignments assignments
    join public.rentals rentals on rentals.id = assignments.rental_id
    where assignments.vehicle_id = v_vehicle_id
      and lower(coalesce(rentals.status, '')) <> 'cancelled'
      and v_transaction.occurred_at >= assignments.assigned_from
      and v_transaction.occurred_at < coalesce(
        assignments.assigned_until, 'infinity'::timestamptz
      )
  ...
  set vehicle_id = v_vehicle_id,
      ...
      review_reason = case
        ...
        when v_candidate_count = 0
          then 'No vehicle-assignment interval contains the transaction occurrence time.'
        when v_candidate_count > 1
          then 'More than one rental assignment contains the transaction occurrence time.'
        else null
      end,
      updated_at = now()
