...
select jsonb_build_object(
  ...
  'held_total_mismatches',(select count(*) from public.rentals r where exists(select 1 from public.rental_deposit_allocations a where a.holder_rental_id=r.id)
    and abs(coalesce(r.deposit_held_amount,0)-(select coalesce(sum(greatest(0,a.amount_held-a.amount_released)),0) from public.rental_deposit_allocations a where a.holder_rental_id=r.id))>0.005),
  'over_released_allocations',(select count(*) from public.rental_deposit_allocations where amount_released>amount_held),
